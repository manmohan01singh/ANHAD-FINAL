...
export function initRAG() {
  ...
  async function search(text) {
    ...
    // Proactive fallback/enrichment: if searchType is English (3), search Romanized (4)
    // for terms that match concepts in CONCEPT_GRAPH.
    if (searchType === 3) {
      const romanTerms = terms.filter(t => {
        const lowerT = t.toLowerCase();
        return Object.values(CONCEPT_GRAPH).some(c =>
          c.en.some(enTerm => enTerm.toLowerCase() === lowerT)
        );
      });
      if (romanTerms.length > 0) {
        promises.push(...romanTerms.slice(0, 3).map(t => searchAPI(t, 4)));
      }
    }
    ...
    // Reactive fallback: if search yielded no verses at all and search type was English, try type 4
    let totalCount = results.reduce((acc, r) => acc + (r ? r.length : 0), 0);
    if (totalCount === 0 && searchType === 3) {
      const fallbackPromises = terms.slice(0, 3).map(t => searchAPI(t, 4));

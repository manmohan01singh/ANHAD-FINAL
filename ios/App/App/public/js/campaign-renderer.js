...
(function () {
  ...
  /**
   * Render a specific campaign regardless of page or schedule window.
   * Exists for the admin screen's Preview, so what an operator previews goes
   * through the exact same render path users get — rather than a lookalike that
   * could drift from it. Never called by the app itself; update() remains the
   * only path that honours isHome() and the resolver.
   */
  function renderPreview(campaign) {
    safe('preview', () => render(campaign));
  }

  window.AnhadCampaignRenderer = { update, clear, renderPreview };
})();

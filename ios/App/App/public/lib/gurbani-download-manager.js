...
(function () {
    ...
    class GurbaniDownloadManager {
        ...
        async showDownloadUI() {
            ...
            // Detect dark mode from global theme system
            const globalTheme = localStorage.getItem('anhad_theme');
            const nitnemOverride = localStorage.getItem('nitnem_theme_override');
            let isDark = false;
            if (nitnemOverride === 'dark') isDark = true;
            else if (globalTheme === 'dark') isDark = true;
            ...
            if (isDark) this.overlay.setAttribute('data-dm', '1');
            ...
            // Add styles — only inject once
            if (!document.getElementById('gurbani-dl-styles')) {
                const styles = document.createElement('style');
                styles.id = 'gurbani-dl-styles';
                styles.textContent = `
                    ...
                    .download-container {
                        ...
                        animation: dlSlideUp 0.5s cubic-bezier(0.34, 1.56, 0.64, 1);
                    }
                    /* Dark mode card */
                    #gurbani-download-overlay[data-dm="1"] .download-container {
                        background: linear-gradient(145deg, rgba(28,28,30,0.97) 0%, rgba(18,18,20,0.95) 100%);
                        border-color: rgba(255,255,255,0.1);
                        box-shadow: 0 8px 32px rgba(0,0,0,0.5), 0 2px 8px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.06);
                    }
                    @keyframes dlSlideUp {
                    ...
                    .download-icon {
                        ...
                        box-shadow: 4px 4px 12px rgba(0,0,0,0.15), inset 0 1px 1px rgba(255,255,255,0.2);
                        flex-shrink: 0;
                    }
                    .download-header-text { flex: 1; }
                    .download-title {
                        color: #1a1a2e; font-size: 20px; margin: 0 0 4px 0;
                        font-weight: 700; letter-spacing: -0.3px;
                    }
                    #gurbani-download-overlay[data-dm="1"] .download-title { color: #F5F5F7; }
                    .download-subtitle { color: #64748b; font-size: 13px; margin: 0; font-weight: 400; }
                    #gurbani-download-overlay[data-dm="1"] .download-subtitle { color: #8E8E93; }
                    .download-steps { display: flex; flex-direction: column; gap: 12px; margin-bottom: 24px; }
                    .download-step {
                        display: flex; align-items: center; gap: 14px; padding: 12px 16px;
                        background: rgba(241, 245, 249, 0.6); border-radius: 14px;
                        transition: all 0.3s ease; border: 1px solid rgba(255,255,255,0.4);
                    }
                    #gurbani-download-overlay[data-dm="1"] .download-step {
                        background: rgba(44,44,46,0.6); border-color: rgba(255,255,255,0.08);
                    }
                    .download-step.active { background: rgba(254, 243, 199, 0.4); border-color: rgba(251, 191, 36, 0.3); }
                    #gurbani-download-overlay[data-dm="1"] .download-step.active {
                    ...
                    .step-number {
                        ...
                        box-shadow: 0 4px 12px rgba(251,191,36,0.35), inset 0 1px 1px rgba(255,255,255,0.4);
                        flex-shrink: 0;
                    }
                    .download-step.active .step-number {
                        background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%);
                        box-shadow: 0 4px 16px rgba(245,158,11,0.45), inset 0 1px 1px rgba(255,255,255,0.4);
                    }
                    .step-text { color: #475569; font-size: 14px; font-weight: 500; }
                    #gurbani-download-overlay[data-dm="1"] .step-text { color: #8E8E93; }
                    .download-step.active .step-text { color: #92400e; font-weight: 600; }
                    #gurbani-download-overlay[data-dm="1"] .download-step.active .step-text { color: #D4AF37; }
                    .download-progress-wrapper {
                        width: 100%; height: 8px; background: rgba(226,232,240,0.6);
                        border-radius: 8px; overflow: hidden; margin-bottom: 16px; padding: 2px;
                    }
                    #gurbani-download-overlay[data-dm="1"] .download-progress-wrapper { background: rgba(255,255,255,0.1); }
                    .download-progress-bar {
                        height: 100%;
                        background: linear-gradient(90deg, #fbbf24 0%, #f59e0b 50%, #fbbf24 100%);
                        width: 0%; transition: width 0.5s cubic-bezier(0.4,0,0.2,1);
                        border-radius: 6px; background-size: 200% 100%;
                        animation: dlShimmer 2s ease-in-out infinite;
                    }
                    @keyframes dlShimmer {
                    ...
                    .download-status { color: #64748b; font-size: 13px; font-weight: 500; }
                    #gurbani-download-overlay[data-dm="1"] .download-status { color: #8E8E93; }
                    .download-count { color: #f59e0b; font-size: 15px; font-weight: 700; }
                    .download-badges { display: flex; gap: 10px; flex-wrap: wrap; }
                    .badge {
                        display: inline-flex; align-items: center; gap: 6px;
                        padding: 8px 14px; background: rgba(254,243,199,0.5); border-radius: 100px;
                        font-size: 12px; font-weight: 600; color: #92400e;
                        border: 1px solid rgba(251,191,36,0.2);
                    }
                    #gurbani-download-overlay[data-dm="1"] .badge {

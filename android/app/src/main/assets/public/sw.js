...
const CACHE_VERSION = 'anhad-v10.33.0'; // v10.33.0: campaign admin write path. Campaign config now comes from a durable store (backend/lib/config-store.js: Cloudflare KV via its REST API when CF_* env vars are set, else a JSON file) instead of a hardcoded const, with admin routes behind the existing requireAdminToken. The read endpoint drops its 5-minute Cache-Control for no-cache + ETag so the 15s poll revalidates cheaply as a 304. Adds an unlisted /Admin/ operator screen (deliberately NOT precached and not linked from anywhere). Verified cross-device: publish on one device reached an already-open device in 5.1s, an offline device kept its cached config unharmed, and disabling reverted everyone automatically.

...
const CACHE_VERSION = 'anhad-v10.35.0'; // v10.35.0: Home lifecycle root-cause fix. trendora-app.js is in SHELL_SCRIPTS and its <script> tag sits outside #app, so it is never re-injected and its IIFE runs once per realm — meaning App.init() was NEVER re-entered on an SPA return to Home, and the only thing still running was refreshAll(), which repaints data but binds nothing. Since smooth-navigation.js replaces #app.innerHTML wholesale, every Practice card and Gurbani Radio hero card came back with no listeners, no role/tabindex, and only a full reload (a new realm) fixed it. App is now split into initOnce() (window/document listeners, realm singletons) + mount() (everything inside #app, re-runnable, per-node bindOnce() guards) + unmount(), driven from refreshHomeForSpa() and a new pageshow probe so hard load / SPA arrival / bfcache all converge on one state. Also: registered the __anhadPageCleanup handler the router has always looked for but nobody ever provided; moved the 6s Gurpurab rotation off a DOM-node property (it leaked one interval per Home visit against a detached #eventCard); collapsed homepage-data.js's two overlapping interval sets; epoch-guarded the Hukamnama async writes that were landing on detached nodes; replaced scroll-engine.js's permanent inline animation:none with a class. Campaign: Chaliya is now an in-greeting rotating announcement that swaps in place of the Guru portraits inside their already-fixed 172px box (zero layout shift) — no X button, no permanent dismissal, and the never-expiring anhad_campaign_dismissed_v1 flag is purged from every device. // v10.34.0: blank-greeting regression fixed properly this time. smooth-navigation.js now bumps a per-arrival epoch instead of the two time-based debounce windows that dropped real arrivals (a <500ms return to Home, or any arrival in the first 2.5s of a session, both skipped the only code that repopulates the greeting/event card). Mini-player layering: one explicit z-index scale replaces ad-hoc escalation, the mini player now defers visually to any open modal/sheet (it was above all of them), and Home reserves 216px of bottom scroll runway only while the player is visible (was 110px always, burying the Gurbani GPT card ~82%). Perf: replaced a 121KB favicon.ico Chrome was fetching implicitly with a 2.5KB one; confirmed no duplicate audio/campaign script injection across repeated navigation.

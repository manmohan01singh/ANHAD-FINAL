...
(function() {
  ...
  if (window.AnhadOverlayPlayerUI) {
    console.log('[GlobalMiniPlayer] Overlay UI already loaded, skipping duplicate init');
    return;
  }
  ...
  function init() {
    console.log('[GlobalMiniPlayer] Initializing thin UI wrapper...');

    if (!window.AnhadAudio) {
      retryCount++;
      if (retryCount >= MAX_RETRIES) {
        console.error('[GlobalMiniPlayer] AnhadAudio singleton not available, aborting');
        return;
      }
      setTimeout(init, 100);
      return;
    ...
    console.log('[GlobalMiniPlayer] ✅ Thin UI wrapper initialized successfully');
  }
  ...
  // Expose global controller mapping so legacy invocations still resolve
  window.GlobalMiniPlayer = {
    play: (s) => window.AnhadAudio.play(s),
    pause: () => window.AnhadAudio.pause(),
    toggle: () => window.AnhadAudio.toggle(),
    stop: () => window.AnhadAudio.stop(),
    isPlaying: () => window.AnhadAudio.isPlaying(),
    getStream: () => window.AnhadAudio.getState().currentStream,
    getAudio: () => window.AnhadAudio.getAudio(),
    show: () => { if (miniPlayerEl) { miniPlayerEl.style.display = 'flex'; miniPlayerEl.classList.add('gmp--visible'); } },
    hide: () => { if (miniPlayerEl) { miniPlayerEl.style.display = 'none'; miniPlayerEl.classList.remove('gmp--visible'); } }
  };
})();

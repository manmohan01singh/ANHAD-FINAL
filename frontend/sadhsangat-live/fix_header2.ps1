...
# Replace the broken header content (lines 4748-4766)
$brokenHeader = @'
    <header class="header">
...
if ($content.Contains($brokenHeader)) {
    $content = $content.Replace($brokenHeader, $goodHeader)
    Write-Host "FOUND and replaced broken header"
} else {
    Write-Host "Broken header text NOT found - checking exact bytes"
    # Try trimming/normalizing
    $idx = $content.IndexOf("id=`"openDrawerBtn`"")
    Write-Host "openDrawerBtn found at index: $idx"
    
    # Try to find the broken part
    $idx2 = $content.IndexOf("</svg>`r`n        </button>`r`n      </div>`r`n    </header>")
    Write-Host "Broken ending found at index: $idx2"
}

[System.IO.File]::WriteAllText('index.html', $content, [System.Text.Encoding]::UTF8)
Write-Host "Done. File size: $((Get-Item 'index.html').Length) bytes"

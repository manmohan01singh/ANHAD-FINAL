...
const CACHE_VERSION = 'anhad-v10.31.0'; // v10.31.0: mobile-only UI audit. The install banner (position:fixed, bottom:0, z-index 100001 from a DUPLICATE --z-banner declaration) sat directly on top of the nav pill (z-index 100000) ~5s after load, hiding navigation and making the lower ~140px of the screen a non-scrollable dead zone; it was also never removed on SPA navigation, so it followed the user onto Insights/Favorites. Invisible above 1024px, which is why every desktop-viewport test passed. Also: mobile-lock.css leaked onto Home via SHARED_STYLESHEET_ALLOWLIST and forced touch-action:pan-y on <html> (no horizontal panning anywhere -> 'scrolling sticks'); gurpurab-special-mode.css redefined --duration-fast 0.2s->2s app-wide; .page-enter animated transform, making #app a containing block for its fixed children so the radio sheet opened below the fold while locking body scroll; three preload.onload sites fired twice for cached images (the 'two-time flash'); homepage-data's fast-return path skipped first-time init entirely; --dynamic-bg-url was cleared on leaving Home and never restored; and the audio singleton was re-injected once per Home visit. // v10.30.0: v10.29.0's staleWhileRevalidate fix was structurally correct but incomplete — found two more real gaps after the user reported the deployed site AND localhost both still showing the old gap/flash. (1) vercel.json has outputDirectory:"frontend", meaning the DEPLOYED site serves every .js/.css/.html directly from Vercel's own static/edge layer — backend/server.js (and every Cache-Control fix made there, including v10.29's) was never in that request path at all. vercel.json only had header rules for /api/*, so Vercel's own default static-asset caching applied completely unopposed on the deployed URL regardless of what this file or server.js said. Added explicit no-cache header rules for .js/.css/.html (and a stronger no-store rule for sw.js itself) directly in vercel.json — this is the actual missing piece for the deployed site. (2) scripts/generate-version.js hardcodes its `version` field as a literal string, and pwa-register.js's update-polling compared THAT field — so if a fix shipped without a developer remembering to hand-bump that literal, version-based polling silently detected no change at all (confirmed: it had stayed "4.8.0" across several real fixes already). The SW's own direct byte-comparison (registration.update(), called on every page load) was a separate, still-working detection path, but this closes the gap in the redundant one: pwa-register.js now compares version.json's `timestamp` field (Date.now(), regenerated fresh on every single run, can't go stale the same way) as the primary change signal. Live-verified: fresh-profile Home/Insights round trip still shows exactly one real refresh (not a reintroduced double-render) with both __anhadPageInit trigger paths now composing correctly, guru slider populates (11 slides), gap measures 110px computed / 109.67px on-screen on a truly fresh load — matches source, confirming the gap was never a code bug, only ever a symptom of stale-served CSS.

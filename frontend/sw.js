...
const CACHE_VERSION = 'anhad-v10.32.0'; // v10.32.0: audio now recovers from a network drop (the engine had zero 'online' handling, the radio page hard-paused on offline and only toasted on reconnect, a MediaError left isPlaying stuck true which blocked every retry, and a sticky errored element was never reset so even manual taps failed). Plus the remote campaign engine's missing consumption layer: lib/remote-config.js already resolved schedule/platform/priority with a remote->cache->builtin fallback but nothing read it. Adds js/campaign-renderer.js + css/campaign.css, enforces the previously-dead config TTL, and polls while foregrounded so an admin toggle reaches devices in <15s. Chaliya ships disabled.
